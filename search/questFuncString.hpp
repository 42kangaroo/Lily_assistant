...
#ifndef SEARCH_QUESTFUNCSTRING_HPP
#define SEARCH_QUESTFUNCSTRING_HPP

#include "questInterface.hpp"

namespace Lily {
    class questFuncString : public questInterface {
    ...
    public:
         ...
         */
        questFuncString(strFuncParam function, std::vector<std::string> words, int priority,
         ...
         * @param how if the recognized sentence must exactly by the same as the Sentence you put into the word variable, use questType::IS
         */
        questFuncString(strFuncParam function, std::vector<std::string> words, int priority, std::string lang,
                        questType how = questType::HAS);

        std::string getAnswer(std::string string) const override;

    private:
        strFuncParam functionParam;
    };
}

#endif //SEARCH_QUESTFUNCSTRING_HPP
